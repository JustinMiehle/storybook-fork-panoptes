			...
			steps: [
				...
				...(options.e2e
					? [
							...
							artifact.persist(
								join(LINUX_ROOT_DIR, WORKING_DIR, "test-results"),
								"test-results",
							),
							testResults.persist(
								join(LINUX_ROOT_DIR, WORKING_DIR, "test-results"),
...
export function defineSandboxFlow<Key extends string>(key: Key) {
	const id = toId(key);
	const data =
		sandboxTemplates.allTemplates[
			key as keyof typeof sandboxTemplates.allTemplates
		];
		...
		() => ({
			...
			steps: [
				...
				...(id.includes("svelte-kit")
					...
					: []),
				artifact.persist(
					`${LINUX_ROOT_DIR}/${SANDBOX_DIR}/${id}/debug-storybook.log`,
					"logs",
				),
...
export function defineSandboxTestRunner(
	sandbox: ReturnType<typeof defineSandboxFlow>,
) {
	return defineJob(
...
export function defineWindowsSandboxDev(
	sandbox: ReturnType<typeof defineSandboxFlow>,
) {
	return defineJob(
		...
		() => ({
			...
			steps: [
				...
				testResults.persist(
					`${WINDOWS_ROOT_DIR}\\${WORKING_DIR}\\test-results`,
				),
			],
		}),
		[sandbox.jobs[0]],
...
export function defineWindowsSandboxBuild(
	sandbox: ReturnType<typeof defineSandboxFlow>,
) {
	return defineJob(
